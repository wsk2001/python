...
class XfcLaPolicy:
    ...
    def __init__(self):
        ...
        self.check_file_closed = ft.Checkbox(label="파일 닫힘 검사", value=True)
        ...
        self.check_cycle = ft.TextField(label="파일 변화(생성) 검사 주기(sec)")
        self.thread_count = ft.TextField(label="작업 Thread 개 수", visible=True)
        ...
        self.ymd_offset = ft.TextField(label="이전 일자 옵셋")

        self.use_trigger_file = ft.Checkbox(label="트리거 확장자 사용 여부", value=False, width=300,
                                            on_change=self.use_trigger_checkbox_changed)
        self.trigger_ext = ft.TextField(label="트리거 파일 확장자", visible=False)
        self.trigger_target = ft.TextField(label="트리거 목적 파일 확장자", visible=False)
